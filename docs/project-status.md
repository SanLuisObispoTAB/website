...
| Item | Owner | Notes |
|---|---|---|
| 🔴 **Which Cheer photos are actually the dance team?** | Cheer team → Erik | The Cheer team says some shots currently on `/teams/dance` are of the **dance** team and need culling, but has not yet said which. **No photo action taken** (#122) — `teamPhoto` is `gdance-team-2025.jpg` (a `gdance-` prefix, so a likely candidate) and the gallery holds `ccheering-sideline-2025.jpg`. Waiting on the specific list before removing anything, since guessing risks pulling legitimate cheer shots. Note this overlaps the **Cheer vs Dance** question below: if Dance becomes its own team, some of these photos belong on *its* page rather than being deleted |
