...
> **Last updated:** 2026-08-11 *(**slotab.org cutover — code side shipped, #100.** The blanket `robots: noindex` is off and indexing is now **host-aware**: slotab.org is indexable, every other host (vercel.app, the SLOHS-firewall alias, previews) gets `X-Robots-Tag: noindex` from the edge proxy. Added `metadataBase` + canonical, `sitemap.xml` (34 URLs) and `robots.txt`. **DNS itself is Erik-side and still pending** — see the Go-Live table. Earlier today: #94–#99.)*
>
