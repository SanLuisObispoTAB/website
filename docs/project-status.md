...
| Item | Owner | Notes |
|---|---|---|
| 🔴 **SLO Credit Union sponsor — logo missing, name and tier to confirm** | Cheer team → Erik | Cheer brought in a **$5,000** sponsorship. Three things block adding it (#125). **(1) No logo.** The intake `.docx` contains **no images at all** — no `word/media/` entry — so the file believed to carry the logo does not. `logo` is a required field on every sponsor entry, so nothing can be added until it arrives; a vector or high-res PNG is best for the wall. **(2) Name.** Reported as "SLO Bank", but slocu.com is **SLO Credit Union** (their own site and page title; 70+ years in SLO County). Credit unions are usually particular about not being called banks — worth getting right on a $5k sponsor's listing. **(3) Tier.** $5,000 is **Gold** on the 2026-27 sheet, not Champion — Champion is $10,000. Gold also carries eight All-Sport passes, scoreboard ads and a two-year banner, so someone should confirm those perks are being delivered |
| 🟡 **Publish athlete rosters, or not? — Tiger Cheer would be first** | Board (policy call) | Cheer supplied a **48-name roster** across Varsity / JV Black / JV Gold with positions and grade levels (#125). **No team on the site currently publishes athlete names** — the `roster` field exists in `TeamPage` but is unused by every team file — so saying yes sets precedent for minors' names + grade on a public page, and other teams will reasonably expect the same. Two sub-questions if approved: whether grade level is included, and that `RosterEntry` needs reshaping since it requires a jersey `number` that cheer athletes don't have (positions are Base / Flyer / Backspot / Tumbler). Also parked: the team's **core values and sayings**, which have no field on the team page yet |
