...
> **Last updated:** 2026-08-11 *(Audited `public/photos` as a library with a new `npm run photo-usage` (perceptual signatures): 6 pages were showing the same photo twice — 4 of them my own #111 imports duplicating photos already present — plus a *basketball* huddle imported as a track photo, and a source file labelled VARSITY that actually contains the JV volleyball team. All fixed; flag football's 4 unused action shots wired up. Pages-showing-twice 6 → 0 — #112. Earlier: reviewed all 180 photos in Erik's two dropped folders and wired in 19 — cross country's hero is now the CIF Championship celebration (retiring its black band and its posed-portrait carousel exception), football's gallery went 0 → 7, and six other galleries grew. `photo-intake` wasn't broken: the folders landed in its output archive, not `photo-inbox/`. Excluded every file with text burned into the image — #111. Earlier: cross country is now the template for the other teams: added `npm run team-audit` + generated `docs/team-page-gaps.md` to track what each page is missing (1 of 17 complete), a random-squad/hover-flip card for co-ed programs, and football + girls volleyball expanded to three captioned squads each from Erik's photo folder. Fixed a cascade bug where the focus-bloom rule silently defeated the crossfade — #110. Earlier: cross country coach bios now come from the SLOHS coaches page — fuller text for both head coaches (Berkeland's stored bio was missing the girls' CIF Championship), plus assistants Kira Abercromby and Tera Dube; fixed two styling bugs the multi-coach block introduced — #109. Earlier: cross country page fixed — both head coaches labelled boys/girls, both squad photos under Meet the Team, and the fake "In Action" portrait removed. Made the action-shot rule consistent across all 19 teams: found 4 portraits in action slots, including baseball and girls-wrestling showing the *same photo twice*. Removing those exposed a contrast bug (white hero text on cream, 2.4:1) — photoless heroes now paint black, 19.8:1 — #108. Earlier: house rule: centered text never ends on a line of fewer than ~4 words — enforced in CSS via inherited `text-wrap: balance`/`pretty` so new copy is covered automatically; `/booster-bash` went from 3 stranded lines to 0 — #107. Hudl venues corrected again after Erik confirmed with Hudl: stadium, Big Gym, baseball and softball fields, **no pool**; `/watch` updated to match — #106. Earlier: Booster Bash page now leads with Comms' 2026 announcement copy and the Tiger's Den logo; added a reusable `intro` field to featured event pages so lead copy sits above the date/venue card, wired into the CMS — #105. Earlier: corrected the Hudl line on `/impact` — it claimed streaming "for every team" when the cameras only cover the stadium, baseball field, gymnasium, and pool; row re-labelled Home Venues / Facilities so the meta tag doesn't re-assert blanket coverage. Flagged that `/watch` names only two of the four venues — #104. Earlier: real FY 2025-26 ledger data shipped to `/impact` — $210,414.79 to SLOHS athletics across 24 programs, reconciled to the penny against the year-end Transaction Report. Clears E_impact / decision #4, open since April. Key finding: the ledger records disbursements, not purchases — 55.6% is per-team allocations with no item detail — so the page was restructured around aggregate totals + named purchases + a name-only programs band; per-team dollar figures deliberately withheld. Coach names on stipends/reimbursements aggregated out; net revenue of −$13,140.17 held back as board-internal. Flagged: #32's "$75K" thermometer anchor is ~2× the real $38,327.90 — #103)*
>
