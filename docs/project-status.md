...
> **Last updated:** 2026-08-11 *(Board roster on `/contact` re-laid out — role hard-left, name + email hard-right, one line per row in a wider 1040px column — #99. Shipped to `main` earlier today: production rollover (#94), real 2026-27 data (#95), fall schedule + two latent bugs (#96), board meeting dates + Hudl status (#97), Trina's Booster Bash notes (#98). `robots: noindex` deliberately still ON — it comes off with the slotab.org DNS cutover.)*
>
