...
> **Last updated:** 2026-08-11 *(Opt-in cookie consent + engagement tracking, board-visible via Vercel Web Analytics — #101. Nothing is stored until a visitor clicks Accept; declining deletes what was stored. New `/privacy` page shows a visitor their own record. Earlier today: slotab.org cutover code side (#100) — **DNS itself still pending, Erik-side**; roster layout (#99); #94–#98.)*
>
...
| G5 | DNS cutover — `slotab.org` → new Vercel project | 🟡 **IN PROGRESS 2026-08-11 — Erik-side, the only remaining launch step.** Add `slotab.org` + `www.slotab.org` as domains on the Raven's Peak Vercel project, then point the registrar records at Vercel. `slotab.org` is already in the Decap postMessage allowlist (#65), so `/admin` keeps working through the switch |
...
| G7 | Cancel GoDaddy Managed WordPress | **48 hours after G5**, once slotab.org is confirmed serving the new site. Destructive and irreversible — Erik-side |
