...
| ❓ **Which GoDaddy account holds slotab.org?** | Board | Erik has access to *a* GoDaddy account but its DNS panel is empty. The domain is GoDaddy-registered on GoDaddy nameservers, so it's either Delegate Access needing an account switch, or a second account (hosting vs registration). See `docs/dns-cutover-runbook.md` (#113) |
