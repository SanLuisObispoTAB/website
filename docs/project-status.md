...
| Item | Owner | Notes |
|---|---|---|
| 🔴 **SLO Credit Union logo — needed as a FILE on disk** | Erik | Name and tier are settled: **SLO Credit Union** (not "SLO Bank") at **Gold** ($5,000), confirmed by Erik 2026-08-16. Still blocked on the artwork. The logo was pasted into chat as an inline image and is **not in either attachment** — the intake `.docx` has no `word/media/` at all, and the XC `.zip` contains no logo file. An image visible in a conversation cannot be written to disk, so it has to arrive as an actual file (save it to `~/Downloads`, or drop it in `photo-inbox/`). Target `public/sponsors/gold/`, matching the existing wall (Logo-Engle-grey.png is 500x171, Cool-cats.jpeg 482x361). A transparent PNG or SVG is best; the supplied square lockup with "CREDIT UNION" beneath the mark is the right asset — preferable to pulling one off their website, which risks a different variant and brand-usage questions |
| 🟡 **17 XC state-meet photos awaiting curation** | Erik + Claude | Staged in gitignored `photo-inbox/` and audited (#126): all 17 need renaming to `<b\|g\|c>XC-<descriptor>`, and only `girls+start` and `coaches+state` identify themselves from the filename. The rest (`DSC084xx`, `State-xx`) need eyes on the image to tell boys from girls — deliberately not guessed, since #112 was exactly that mistake. Two need resizing (`girls+start` 2834x1889, `coaches+state` 6.7 MB); the rest are already 1200px. Say the word and I'll view and classify them, or hand back a contact sheet for you to label |
...
| 🟡 **Rest of the 2026-27 SLOTAB events** | Board | ✅ Board meetings done (#97/#98) — all ten, Aug 2026–May 2027; August moved to Mon Aug 10 and is recorded as held, venue confirmed. *Still needed:* **fall parent meeting**, **physicals night**, **spring fundraiser** |
