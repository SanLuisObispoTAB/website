...
> **Last updated:** 2026-08-11 *(🔴 **`slotab.org` expires 2026-09-19 — 39 days out; confirm auto-renew before anything else.** Found while diagnosing the empty GoDaddy DNS panel — the domain is GoDaddy-registered on GoDaddy nameservers, so that's a Delegate-Access/account issue rather than a DNS one; also confirmed **no MX records**, so the cutover cannot break email — #113. Cutover runbook: `docs/dns-cutover-runbook.md`.)*
>
